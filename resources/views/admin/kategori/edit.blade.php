                ...
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <input type="text" id="kode_kategori" name="kode_kategori"
                            ...
                            class="w-full px-4 py-2.5 text-gray-700 bg-gray-100 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all @error('kode_kategori') border-red-500 @enderror"
                            readonly>
                        @error('kode_kategori')
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea id="deskripsi" name="deskripsi" rows="4"
                            ...
                            class="w-full px-4 py-2.5 text-gray-700 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent transition-all @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi', $kategori->deskripsi) }}</textarea>
                        @error('deskripsi')
                            <p class="mt-1 text-sm text-red-500">
                                <i class="mr-1 fas fa-exclamation-circle"></i>
                                {{ $message }}
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select id="icon" name="icon"
                                ...
                                <option value="fa-rice" {{ old('icon', $kategori->icon) == 'fa-rice' ? 'selected' : '' }}>🌾 Beras</option>
                                <option value="fa-oil-can" {{ old('icon', $kategori->icon) == 'fa-oil-can' ? 'selected' : '' }}>🛢️ Minyak</option>
                                <option value="fa-cube" {{ old('icon', $kategori->icon) == 'fa-cube' ? 'selected' : '' }}>🧊 Gula</option>
                                <option value="fa-pepper-hot" {{ old('icon', $kategori->icon) == 'fa-pepper-hot' ? 'selected' : '' }}>🌶️ Bumbu</option>
                                <option value="fa-cookie-bite" {{ old('icon', $kategori->icon) == 'fa-cookie-bite' ? 'selected' : '' }}>🍪 Makanan Ringan</option>
                                <option value="fa-coffee" {{ old('icon', $kategori->icon) == 'fa-coffee' ? 'selected' : '' }}>☕ Minuman</option>
                                <option value="fa-bread-slice" {{ old('icon', $kategori->icon) == 'fa-bread-slice' ? 'selected' : '' }}>🍞 Roti</option>
                                <option value="fa-soap" {{ old('icon', $kategori->icon) == 'fa-soap' ? 'selected' : '' }}>🧼 Perlengkapan</option>
                            </select>
                        ...
                        <div class="flex items-center justify-center w-full p-4 mt-3 border-2 border-gray-200 border-dashed rounded-lg bg-gray-50">
                            <div class="text-center">
                                <div class="flex items-center justify-center w-16 h-16 mx-auto mb-2 bg-green-100 rounded-lg">
                                    <i id="iconDisplay" class="text-2xl text-green-600 fas {{ $kategori->icon ?? 'fa-icons' }}"></i>
                                </div>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <select id="warna" name="warna"
                                ...
                                <option value="orange" {{ old('warna', $kategori->warna) == 'orange' ? 'selected' : '' }}>🟠 Orange</option>
                        ...
                        <div class="flex items-center justify-center w-full p-4 mt-3 border-2 border-gray-200 border-dashed rounded-lg bg-gray-50">
                            <div class="text-center">
                                <div id="colorDisplay" class="w-16 h-16 mx-auto mb-2 border-4 border-white rounded-lg shadow-md bg-gradient-to-br 
                                    @if($kategori->warna == 'orange') from-orange-400 to-orange-600
                                    @elseif($kategori->warna == 'blue') from-blue-400 to-blue-600
                                    @elseif($kategori->warna == 'red') from-red-400 to-red-600
                                    @elseif($kategori->warna == 'purple') from-purple-400 to-purple-600
                                    @elseif($kategori->warna == 'cyan') from-cyan-400 to-cyan-600
                                    @elseif($kategori->warna == 'yellow') from-yellow-400 to-yellow-600
                                    @elseif($kategori->warna == 'pink') from-pink-400 to-pink-600
                    ...
                    <div class="flex items-start">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-blue-700">
                                Pastikan perubahan data sudah benar sebelum menyimpan. Perubahan akan mempengaruhi semua produk yang terkait dengan kategori ini.
                            </p>
                        </div>
                    </div>
                </div>
...
@push('scripts')
    <script>
        ...
        document.getElementById('formKategori').addEventListener('submit', function(e) {
            ...
            if (!confirm('Apakah Anda yakin ingin memperbarui data kategori ini?')) {
                e.preventDefault();
                return false;
            }
        });
