...
@else
    <a href="#" data-page="{{ $kasirs->currentPage() - 1 }}"
...
@foreach ($kasirs->getUrlRange(1, $kasirs->lastPage()) as $page => $url)
    <a href="#" data-page="{{ $page }}"
        class="pagination-link px-4 py-2 text-sm font-medium border rounded-lg
        {{ $page == $kasirs->currentPage() ? 'text-white bg-green-600 border-green-600 hover:bg-green-700' : 'text-gray-700 bg-white border-gray-300 hover:bg-gray-50' }}">
        {{ $page }}
    </a>
@endforeach

@if ($kasirs->hasMorePages())
    <a href="#" data-page="{{ $kasirs->currentPage() + 1 }}"
        class="px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg pagination-link hover:bg-gray-50">
